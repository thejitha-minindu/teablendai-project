...
import { getAnalyticsSales } from "@/services/dashboard/analyticsService";
...
export function useAnalyticsSales(refreshMs: number = DEFAULT_REFRESH_MS) {
  const [data, setData] = useState<AnalyticsSalesResponse | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const payload = await getAnalyticsSales(false);
        if (!mounted) return;
        setData(payload);
        ...
        if (!mounted) return;
        const msg = err instanceof Error ? err.message : "Failed to load sales analytics";
        setError(msg);
        setIsStale(true);
      } finally {
