...
import { getAnalyticsBlends } from "@/services/dashboard/analyticsService";
...
export function useAnalyticsBlends(refreshMs: number = DEFAULT_REFRESH_MS) {
  const [data, setData] = useState<AnalyticsBlendsResponse | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const payload = await getAnalyticsBlends(false);
        if (!mounted) return;
        setData(payload);
        ...
        if (!mounted) return;
        const msg = err instanceof Error ? err.message : "Failed to load blend analytics";
        setError(msg);
        setIsStale(true);
      } finally {
