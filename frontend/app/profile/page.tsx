...
export default function UserProfilePage() {
  ...
  const [violatorSuggestions, setViolatorSuggestions] = useState<string[]>([]);
  const [reportFilter, setReportFilter] = useState<'all' | 'open' | 'under_review' | 'resolved' | 'closed'>('all');
  const [reportSort, setReportSort] = useState<'newest' | 'oldest'>('newest');
  const [reportStatusFilter, setReportStatusFilter] = useState<ReportItem['status'] | 'all'>('all');

  // ── Notifications state (real API) ───────────────────────────────────────
    ...
    const loadViolations = async () => {
      ...
      try {
        ...
        const mapped: ReportItem[] = response.data.map((v: any) => ({
          id: v.violation_id,
          subject: v.violation_type,
          category: v.violation_type,
  ...
  useEffect(() => {
    const loadViolatorSuggestions = async () => {
      try {
        const response = await apiClient.get('/users');
        const suggestions = response.data
          .map((user: any) => user.user_id ?? user.id ?? user.uuid ?? user.username)
          .filter(Boolean);
        setViolatorSuggestions(suggestions);
      } catch (error) {
        console.error('Failed to load violator suggestions:', error);
      }
    };

    loadViolatorSuggestions();
  }, []);
  ...
  const handleReportSubmit = async () => {
    ...
    try {
      const response = await apiClient.post('/violations', {
        violator_id: reportForm.violatorId,
        auction_id: reportForm.auctionId || null,
        violation_type: reportForm.violationType || 'Other',
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div className="w-full sm:w-auto">
              {isEditing ? (
                <div className="flex gap-2 w-full">
                  <button onClick={() => setIsEditing(false)} className="flex-1 sm:flex-none px-4 py-2 text-sm font-bold text-gray-600 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">
                    Cancel
                  </button>
          ...
          <div className="p-6 md:p-8">
                ...
                <div className="pt-8 border-t">
                  ...
                  {showChangePassword && (
                    <div className="mt-4 p-5 border rounded-2xl bg-gray-50 grid grid-cols-1 md:grid-cols-3 gap-4 animate-in fade-in slide-in-from-top-2">
                      <input type="password" placeholder="Current Password" className="p-2.5 border rounded-lg bg-white outline-none focus:ring-2 focus:ring-[#588157]" onChange={(e) => setPasswords({ ...passwords, current: e.target.value })} />
                      <input type="password" placeholder="New Password" className="p-2.5 border rounded-lg bg-white outline-none focus:ring-2 focus:ring-[#588157]" onChange={(e) => setPasswords({ ...passwords, new: e.target.value })} />
                      <input type="password" placeholder="Confirm Password" className="p-2.5 border rounded-lg bg-white outline-none focus:ring-2 focus:ring-[#588157]" onChange={(e) => setPasswords({ ...passwords, confirm: e.target.value })} />
                      <button onClick={handlePasswordUpdate} disabled={pwdLoading} className="bg-[#588157] text-white rounded-lg font-bold hover:bg-[#4a6d49] transition-colors disabled:bg-gray-400 md:col-span-3">
                      ...
                      {(() => {
                        ...
                        return weeklySales.map((item, i) => (
                          <div key={i} className="group flex-1 flex flex-col items-center relative h-full justify-end">
                            ...
                            <div style={{ height: `${(item.value / max) * 100}%` }} className="w-full bg-[#588157] rounded-t-md transition-all duration-500 group-hover:bg-[#a3b18a] cursor-pointer" />
            ...
            {/* ── 4. REPORTS TAB — wired to /violations/ API ── */}
            {activeTab === 'reports' && (
              <div className="space-y-6">
                ...
                {reportSubTab === 'submit' && (
                  <div className="p-6 bg-gray-50 border border-gray-200 rounded-2xl space-y-4">
                    <div className="space-y-1">
                      ...
                      />
                      <datalist id="violator-suggestions">
                        {violatorSuggestions.map((id) => (
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        type="text"
                        placeholder="Enter Auction ID (optional)"
                ...
                {reportSubTab === 'history' && (
                  <div className="space-y-3">
                    {reportsLoading && (
                      <p className="text-sm text-gray-400 text-center py-6">Loading reports...</p>
                    )}
            ...
            {/* ── 5. NOTIFICATIONS TAB — wired to /notifications/ API ── */}
            {activeTab === 'notifications' && (
              <div className="space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <p className="text-sm text-gray-500">
                    {notifLoading ? 'Loading...' : unreadCount > 0
                      ? <span className="font-semibold text-gray-700">{unreadCount} unread</span>
                      : 'All caught up!'}
                    {!notifLoading && unreadCount > 0 && ' — click a notification to mark it as read.'}
                  </p>
