      ...
      <div className={`bg-white max-w-md w-full rounded-2xl shadow-xl p-8 text-center border-t-8 border-green-500 transition-all duration-700 transform ${showContent ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
        ...
        <div className="space-y-3">
          <button
            ...
            disabled={!orderId}
            className={`w-full ${orderId ? 'bg-[#588157] hover:bg-[#3A5A40]' : 'bg-gray-200 cursor-not-allowed'} text-white font-bold py-3 rounded-xl transition-all shadow-md flex items-center justify-center gap-2 ${!orderId ? 'opacity-60' : ''}`}
          >
