...
def list_orders(user_id: Optional[str] = None, status: Optional[str] = None, service: OrderService = Depends(get_order_service)):
    return service.list_orders(user_id=user_id, status=status)

# Get all orders for a specific user
@router.get("/user/{user_id}", response_model=List[Order])
def get_orders_by_user(user_id: str, service: OrderService = Depends(get_order_service)):
    return service.list_orders_by_user(user_id=user_id)
...
def create_wins_auction(wins_auction: WinsAuction, service: WinsAuctionService = Depends(get_wins_auction_service)):
...
def get_wins_auction(auction_id: str, user_id: str, service: WinsAuctionService = Depends(get_wins_auction_service)):
    wins_auction = service.get_wins_auction(auction_id=auction_id, user_id=user_id)
    if not wins_auction:
...
def list_wins_auctions(user_id: Optional[str] = None, auction_id: Optional[str] = None, service: WinsAuctionService = Depends(get_wins_auction_service)):
    return service.list_wins_auctions(user_id=user_id, auction_id=auction_id)

# Get all wins auctions for a specific user
@router.get("/wins/user/{user_id}", response_model=List[WinsAuction])
def get_wins_auctions_by_user(user_id: str, service: WinsAuctionService = Depends(get_wins_auction_service)):
    return service.list_wins_auctions_by_user(user_id=user_id)

# Get all wins records for a specific auction
@router.get("/wins/auction/{auction_id}", response_model=List[WinsAuction])
def get_wins_auctions_by_auction(auction_id: str, service: WinsAuctionService = Depends(get_wins_auction_service)):
